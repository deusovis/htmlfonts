...
try:
    ...
    for slug, title, desc, code in top_guides:
        ...
        html = f"""<!DOCTYPE html><html lang="en"><head><title>{title} | htmlfonts Guides</title><meta name="description" content="{desc}"><link rel="icon" href="/favicon.svg" type="image/svg+xml"><script src="[https://cdn.tailwindcss.com](https://cdn.tailwindcss.com)"></script><style>body {{ font-family: system-ui, sans-serif; }}</style></head><body class="bg-slate-50 min-h-screen flex flex-col selection:bg-indigo-200"><header class="bg-white/90 backdrop-blur-md border-b border-slate-200 sticky top-0 z-40 shadow-sm"><div class="max-w-7xl mx-auto px-4 h-16 flex justify-between items-center"><a href="/" class="flex items-center gap-2 font-black text-2xl"><span class="text-indigo-600">html</span>fonts</a><nav class="hidden md:flex space-x-8 text-xs font-bold uppercase tracking-widest text-slate-500"><a href="/">Directory</a><a href="/font-vs-font-comparison-tool.html">Font VS Font</a><a href="/daily-css-typography-tips.html">Editor's Desk</a><a href="/html-css-font-guides.html">Guides</a></nav></div></header><main class="flex-grow py-16 px-6"><div class="max-w-3xl mx-auto"><a href="/html-css-font-guides.html" class="text-indigo-600 font-bold uppercase tracking-widest text-xs">&larr; Back to Guides</a><article class="bg-white p-10 mt-8 rounded-3xl shadow-lg border border-slate-200"><h1 class="text-4xl font-black mb-4 tracking-tight text-slate-900">{title}</h1><p class="text-xl text-slate-600 mb-8">{desc}</p><div class="bg-slate-900 p-6 rounded-xl overflow-x-auto"><code class="text-indigo-300 font-mono text-sm whitespace-pre">{safe_code}</code></div></article></div></main></body></html>"""
    ...
    # Comparisons (Updated with Purple/Indigo colors and System Font logic)
