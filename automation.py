...
try:
    ...
    # Safely strip markdown formatting to prevent unterminated string errors
    ...
    sitemap += '  <url><loc>[https://htmlfonts.com/editors-desk.html](https://htmlfonts.com/editors-desk.html)</loc><priority>0.9</priority></url>\n'

    # 2. CONTENT DEFINITIONS (Full Arrays to prevent Git empty folder errors)
    top_guides = [
    ...
    for slug, title, desc, code in top_guides:
        ...
        html = f"""<!DOCTYPE html><html lang="en"><head><title>{title} | htmlfonts Guides</title><meta name="description" content="{desc}"><link rel="icon" href="/favicon.svg" type="image/svg+xml"><script src="[https://cdn.tailwindcss.com](https://cdn.tailwindcss.com)"></script><style>body {{ font-family: system-ui, sans-serif; }}</style></head><body class="bg-slate-50 min-h-screen flex flex-col selection:bg-indigo-200"><header class="bg-white/90 backdrop-blur-md border-b border-slate-200 sticky top-0 z-40 h-16 flex items-center px-6"><div class="max-w-7xl mx-auto w-full flex justify-between items-center"><a href="/" class="font-black text-2xl group"><span class="text-indigo-600">html</span>fonts</a><nav class="hidden md:flex space-x-8 text-xs font-bold uppercase tracking-widest text-slate-500"><a href="/" class="hover:text-indigo-600 transition">Directory</a><a href="/font-vs-font-comparison-tool.html" class="hover:text-indigo-600 transition">Font VS Font</a><a href="/editors-desk.html" class="hover:text-indigo-600 transition">Editor's Desk</a></nav></div></header><main class="flex-grow py-16 px-6"><div class="max-w-3xl mx-auto"><a href="/" class="text-indigo-600 font-bold uppercase tracking-widest text-xs">&larr; Back to Directory</a><article class="bg-white p-10 mt-8 rounded-3xl shadow-lg border border-slate-200"><h1 class="text-4xl font-black mb-4 tracking-tight text-slate-900">{title}</h1><p class="text-xl text-slate-600 mb-8">{desc}</p><div class="bg-slate-900 p-6 rounded-xl overflow-x-auto"><code class="text-indigo-300 font-mono text-sm whitespace-pre">{safe_code}</code></div></article></div></main><footer class="bg-white border-t py-12 text-center text-xs font-bold text-slate-500 uppercase tracking-widest"><p>&copy; {datetime.datetime.now().year} htmlfonts</p></footer></body></html>"""
    ...
    for font_a, font_b, css_a, css_b, link_a, link_b in top_comparisons:
        ...
        vs_html = f"""<!DOCTYPE html><html lang="en"><head><meta charset="UTF-8"><meta name="viewport" content="width=device-width, initial-scale=1.0"><title>{font_a} vs {font_b} | Side-by-Side Comparison</title><link rel="icon" href="/favicon.svg" type="image/svg+xml"><script src="[https://cdn.tailwindcss.com](https://cdn.tailwindcss.com)"></script>{imp_a}{imp_b}<style>body {{ font-family: system-ui, sans-serif; }} .toast-active {{ opacity: 1; transform: translate(-50%, 0); transition: all 0.3s; }}</style></head><body class="bg-slate-50 min-h-screen flex flex-col selection:bg-indigo-200"><div id="toast" class="fixed bottom-10 left-1/2 transform -translate-x-1/2 hidden bg-slate-900 text-white px-8 py-4 rounded-2xl shadow-2xl z-[100] text-sm font-black uppercase">Copied! 🚀</div><header class="bg-white/90 backdrop-blur-md border-b border-slate-200 sticky top-0 z-40 h-16 flex items-center px-6"><div class="max-w-7xl mx-auto w-full flex justify-between items-center"><a href="/" class="font-black text-2xl group"><span class="text-indigo-600">html</span>fonts</a><nav class="hidden md:flex space-x-8 text-xs font-bold uppercase tracking-widest text-slate-500"><a href="/" class="hover:text-indigo-600 transition">Directory</a><a href="/font-vs-font-comparison-tool.html" class="hover:text-indigo-600 transition">Font VS Font</a><a href="/editors-desk.html" class="hover:text-indigo-600 transition">Editor's Desk</a></nav></div></header><main class="flex-grow py-16 px-4"><div class="max-w-6xl mx-auto w-full"><div class="text-center mb-12"><a href="/font-vs-font-comparison-tool.html" class="text-indigo-600 font-bold text-xs uppercase tracking-widest">&larr; Back to Tool</a><h1 class="text-5xl font-black mt-6 tracking-tight text-slate-900">{font_a} <span class="text-slate-300">vs</span> {font_b}</h1></div><div class="bg-white rounded-3xl p-10 shadow-2xl border border-slate-200/60"><input type="text" id="vs-text" value="Optimize your UI design with fast-loading fonts." onfocus="if(this.value==='Optimize your UI design with fast-loading fonts.') this.value=''" onblur="if(this.value==='') this.value='Optimize your UI design with fast-loading fonts.'" oninput="u()" class="w-full mb-10 px-6 py-4 bg-slate-50 border rounded-xl text-xl text-center font-medium shadow-inner outline-none focus:ring-2 focus:ring-indigo-500 transition-all"><div class="grid grid-cols-1 md:grid-cols-2 gap-10 divide-y md:divide-y-0 md:divide-x divide-slate-100"><div><div class="flex items-center gap-2 mb-6"><span class="bg-indigo-600 text-white text-[10px] font-black px-2 py-1 rounded">A</span><h3 class="text-2xl font-black">{font_a}</h3></div><div class="bg-indigo-50/20 p-6 rounded-2xl border border-indigo-100/50 min-h-[200px] flex items-center"><p id="pa" class="text-5xl leading-tight text-indigo-900 w-full" style="font-family: {css_a};">Optimize your UI design with fast-loading fonts.</p></div><div class="mt-8 bg-slate-900 p-4 rounded-xl relative group text-left"><code id="ha" class="text-xs text-indigo-300 font-mono block">{html_a}</code>{f'<button onclick="c(\\'ha\\')" class="absolute top-2 right-2 bg-indigo-600 text-white px-2 py-1 rounded text-[10px] opacity-0 group-hover:opacity-100 transition">COPY</button>' if link_a else ''}</div><div class="mt-4 bg-slate-900 p-4 rounded-xl relative group text-left"><code id="ca" class="text-xs text-indigo-300 font-mono">font-family: {css_a};</code><button onclick="c('ca')" class="absolute top-2 right-2 bg-indigo-600 text-white px-2 py-1 rounded text-[10px] opacity-0 group-hover:opacity-100 transition">COPY</button></div></div><div><div class="flex items-center gap-2 mb-6 md:ml-10"><span class="bg-violet-600 text-white text-[10px] font-black px-2 py-1 rounded">B</span><h3 class="text-2xl font-black">{font_b}</h3></div><div class="md:ml-10 bg-violet-50/20 p-6 rounded-2xl border border-violet-100/50 min-h-[200px] flex items-center"><p id="pb" class="text-5xl leading-tight text-violet-900 w-full" style="font-family: {css_b};">Optimize your UI design with fast-loading fonts.</p></div><div class="md:ml-10 mt-8 bg-slate-900 p-4 rounded-xl relative group text-left"><code id="hb" class="text-xs text-indigo-300 font-mono block">{html_b}</code>{f'<button onclick="c(\\'hb\\')" class="absolute top-2 right-2 bg-indigo-600 text-white px-2 py-1 rounded text-[10px] opacity-0 group-hover:opacity-100 transition">COPY</button>' if link_b else ''}</div><div class="md:ml-10 mt-4 bg-slate-900 p-4 rounded-xl relative group text-left"><code id="cb" class="text-xs text-indigo-300 font-mono">font-family: {css_b};</code><button onclick="c('cb')" class="absolute top-2 right-2 bg-indigo-600 text-white px-2 py-1 rounded text-[10px] opacity-0 group-hover:opacity-100 transition">COPY</button></div></div></div></div></div></main><footer class="bg-white border-t py-12 text-center text-xs font-bold text-slate-500 uppercase tracking-widest"><p>&copy; {datetime.datetime.now().year} htmlfonts</p></footer><script>function u() {{ const v = document.getElementById('vs-text').value; document.getElementById('pa').innerText = v; document.getElementById('pb').innerText = v; }} function c(id) {{ const t = document.getElementById(id).textContent; navigator.clipboard.writeText(t).then(() => {{ const ts = document.getElementById('toast'); ts.classList.remove('hidden'); setTimeout(() => ts.classList.add('toast-active'), 10); setTimeout(() => {{ ts.classList.remove('toast-active'); setTimeout(() => ts.classList.add('hidden'), 300); }}, 3000); }}); }}</script></body></html>"""
    ...
    tip_html = f"""<!DOCTYPE html><html lang="en"><head><title>{new_data['title']} | Editor's Desk</title><meta name="description" content="{new_data['tip']}"><link rel="icon" href="/favicon.svg" type="image/svg+xml"><script src="[https://cdn.tailwindcss.com](https://cdn.tailwindcss.com)"></script><style>body {{ font-family: system-ui, sans-serif; }}</style></head><body class="bg-slate-50 min-h-screen flex flex-col font-sans selection:bg-indigo-200"><header class="bg-white/90 backdrop-blur-md border-b border-slate-200 sticky top-0 z-40 h-16 flex items-center px-6"><div class="max-w-7xl mx-auto w-full flex justify-between items-center"><a href="/" class="font-black text-2xl"><span class="text-indigo-600">html</span>fonts</a><nav class="hidden md:flex space-x-8 text-xs font-bold uppercase tracking-widest text-slate-500"><a href="/">Directory</a><a href="/font-vs-font-comparison-tool.html">Font VS Font</a><a href="/editors-desk.html" class="text-indigo-600">Editor's Desk</a></nav></div></header><main class="flex-grow py-16 px-6"><div class="max-w-3xl mx-auto"><a href="/editors-desk.html" class="text-indigo-600 font-bold uppercase tracking-widest text-xs">&larr; Back to Archive</a><article class="bg-white p-10 mt-8 rounded-3xl shadow-lg border border-slate-200"><span class="text-indigo-600 text-xs font-black uppercase tracking-widest">{new_data['date']}</span><h1 class="text-4xl font-black mt-2 mb-4 tracking-tight text-slate-900">{new_data['title']}</h1><p class="text-xl text-slate-600 mb-8 font-medium leading-relaxed">{new_data['tip']}</p><div class="bg-slate-900 p-8 rounded-2xl overflow-x-auto border border-slate-800"><pre class="text-indigo-300 font-mono text-sm"><code>{tip_safe_code}</code></pre></div></article></div></main><footer class="bg-white border-t py-12 text-center text-xs font-bold text-slate-500 uppercase tracking-widest"><p>&copy; {datetime.datetime.now().year} htmlfonts</p></footer></body></html>"""
    ...
    archive_cards = ""
    for item in history:
        archive_cards += f"""<a href="/article/{item['slug']}.html" class="block bg-white p-8 rounded-3xl shadow-sm border border-slate-200 hover:shadow-xl hover:border-indigo-300 transition-all group"><span class="text-xs font-black text-indigo-600 uppercase tracking-widest">{item['date']}</span><h3 class="text-2xl font-black text-slate-900 mt-2 group-hover:text-indigo-600 transition-colors">{item['title']}</h3><p class="text-slate-500 mt-3 font-medium leading-relaxed">{item['tip']}</p></a>"""
    
    archive_html = f"""<!DOCTYPE html><html lang="en"><head><title>Editor's Desk Archive | htmlfonts</title><meta name="description" content="Daily CSS typography tips and web design insights."><link rel="icon" href="/favicon.svg" type="image/svg+xml"><script src="[https://cdn.tailwindcss.com](https://cdn.tailwindcss.com)"></script><style>body {{ font-family: system-ui, sans-serif; }}</style></head><body class="bg-slate-50 min-h-screen flex flex-col font-sans selection:bg-indigo-200"><header class="bg-white/90 backdrop-blur-md border-b border-slate-200 sticky top-0 z-40 h-16 flex items-center px-6"><div class="max-w-7xl mx-auto w-full flex justify-between items-center"><a href="/" class="font-black text-2xl"><span class="text-indigo-600">html</span>fonts</a><nav class="hidden md:flex space-x-8 text-xs font-bold uppercase tracking-widest text-slate-500"><a href="/">Directory</a><a href="/font-vs-font-comparison-tool.html">Font VS Font</a><a href="/editors-desk.html" class="text-indigo-600">Editor's Desk</a></nav></div></header><main class="flex-grow py-16 px-6 max-w-4xl mx-auto w-full"><h1 class="text-5xl font-black tracking-tight text-slate-900 mb-4">Editor's Desk Archive</h1><p class="text-xl text-slate-500 mb-12">Every tip, snippet, and typography insight we've ever published.</p><div class="space-y-6">{archive_cards}</div></main><footer class="bg-white border-t py-12 text-center text-xs font-bold text-slate-500 uppercase tracking-widest"><p>&copy; {datetime.datetime.now().year} htmlfonts</p></footer></body></html>"""
...
try:
    ...
    # Safe tweet formatting to avoid markdown links triggering 403s
